...
Template.bookingPrintShow.created = function () {
  ...
  this.autorun(function () {
    ...
    Meteor.subscribe('customerByBooking', Router.current().params._id);

    Meteor.subscribe('equipmentCalendarsStartAndEnd', Session.get('startMonth2'), Session.get('endMonth2'), null, function () {
      
    });
    Meteor.subscribe('calendarsStartAndEnd', Session.get('startMonth2'), Session.get('endMonth2'), null, function () {
    ...
    var bookingLineItems = BookingLineItems.find({invoiceId: Router.current().params._id}).fetch(); 
      ...
      for(y in bookingLineItems) {
          var obj = new Object();
          if(bookingLineItems[y].unclashableSerialNumbers.length > 0 || bookingLineItems[y].clashableSerialNumbers.length > 0) {
            // obj.item = booking.equipmentDetails[x].items[y];
            // obj.serialNoArray = booking.equipmentDetails[x].items[y].unclashableSerialNumbers;
            // obj.serialNoArray = obj.serialNoArray.concat(booking.equipmentDetails[x].items[y].clashableSerialNumbers);
            itemsArray.push(bookingLineItems[y]);
          }
      }

    Session.setTemp("itemsArray", itemsArray);
...
Template.bookingPrintShow.rendered = function () {
      // $( "div.selectize-input > input" ).addClass( "myClass yourClass" );
      var booking = Bookings.findOne({_id: Router.current().params._id});
      var bookingStatus = booking.status;
      if(bookingStatus == "Void") {
        Session.setTemp("void", true);
      }
      ...
      for(z in booking.customerSignOut) {
        wrapper = document.getElementById("signature-pad-"+z);
        ...
        signaturePad.fromDataURL(booking.customerSignOut[z].image);
        signaturePad.off();
      }

      for(a in booking.staffSignIn) {
        wrapper = document.getElementById("signature-pad-staff-"+a);
        canvas = wrapper.querySelector("canvas");
        ...
        signaturePad = new SignaturePad(canvas);
        signaturePad.fromDataURL(booking.staffSignIn[a].image);
        signaturePad.off();
      }
};

Template.bookingPrintShow.helpers({
  ...
  icStatus: function() {
    return (Customers.findOne({_id: this._id}).icStatus);
  },
  needUpdate: function() {
  ...
  finalSubTotal: function() {
    ...
    var finalSubTotal = 0;
    for(x in booking.equipmentDetails) {
      finalSubTotal = parseFloat(finalSubTotal + booking.equipmentDetails[x].subTotal);
    }
  ...
  projectName: function() {
    if(BookingProjects.findOne({invoiceId: Router.current().params._id}) != undefined) {
      if(BookingProjects.findOne({invoiceId: Router.current().params._id}).projectName == "" || BookingProjects.findOne({invoiceId: Router.current().params._id}).projectName == null) {
        return "Nil";
      }
  ...
  subAmount: function() {
    return accounting.formatMoney((this.discount + this.rate) * this.days * this.booked);
  },
...
Template.bookingPrintShow.events({
  'click #printInvoice': function (event) {
          ...
          cordova.plugins.printer.print(printData, 'bookingPrintShow.html', function () {
