...
Template.quotations.destroyed = function () {
...
Template.quotations.rendered = function () {
...
Template.quotations.helpers({
  ...
  bookings: function () {
    var bookingStatuses = BookingStatuses.find({type: "Quotation"}).fetch();
...
Template.quotations.events({
  ...
  'click #submitqbinvoiceid': function(e) {
    ...
    if(BookingStatuses.findOne({quickbooksInvoiceId: $("#qbinvoiceidtext").val()}) != undefined) {
      Router.go('quotations.show', {_id: BookingStatuses.findOne({quickbooksInvoiceId: $("#qbinvoiceidtext").val()}).invoiceId}, {});
    } else {
  ...
  'click #addQuotation': function(event, template) {
